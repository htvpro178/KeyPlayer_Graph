...
void printResult(const JsonObject &result) {
	JsonArray vertices = result["vertices"].toList();
	int i = 1;
	foreach(QVariant vertex, vertices) {
		JsonObject name = vertex.toMap();
		cout << "Name-" << i << name["Name"].toInt() << endl;
		i++;
	}

	JsonArray edges = result["edges"].toList();
	foreach(QVariant edge, edges) {
		JsonObject edgeValue = edge.toMap();
		cout << "Start-" << edgeValue["Start"].toInt()<<endl;
		cout << "End-" << edgeValue["End"].toInt() << endl;
		cout << "DirectInfluence-" << edgeValue["DirectInfluence"].toFloat() << endl;
	}
}

void Parser::ParseJsonValue()
{
	// Read JSON file
	QString json = readFile("C:/testkp/graph5.json");
	if (json.isEmpty()) {
...
*/
bool Parser::loadAdjacencyMatrix(){
    ...
    while ( !ts.atEnd() )   {
        ...
        j=0;
		float weightsource = 1.0;
		QStringList::Iterator it1 = lineElement.begin() + i;
		weightsource = (*it1).toFloat(&intOK);
        for (QStringList::Iterator it2 = lineElement.begin(); it2!=lineElement.end(); ++it2)   
		{
            if ( (*it2)!="0" && i!=j)
			{
				float tempWeight = 0.0;
                tempWeight =(*it2).toFloat(&intOK);
				if (tempWeight > 0)
				{
					edgeWeight = tempWeight/float(weightsource);
					undirected=0;
					arrows=true;
					bezier=false;
					//emit createEdge1(i+1, j+1, tempWeight, edgeWeight, initEdgeColor, undirected, arrows, bezier);
...
bool Parser::loadAdjacencyJson() {
	...
	foreach(QVariant vertex, vertices) {
		JsonObject name = vertex.toMap();
		cout << "Name-" << node << name["Name"].toInt() << endl;
		node++;
	}
	aNodes = node;
	...
	foreach(QVariant edge, edges) {
		...
		edgeWeight = edgeValue["DirectInfluence"].toFloat();
		undirected = 0;
		arrows = true;
		bezier = false;
		emit createEdge(edgeValue["Start"].toInt(), edgeValue["End"].toInt(), edgeWeight, initEdgeColor, undirected, arrows, bezier);
		totalLinks++;
		qDebug() << "Parser-loadAdjacency(): Link from i=" << edgeValue["Start"].toInt() << " to j=" << edgeValue["End"].toInt()
			<< "weight=" << edgeWeight << ". TotalLinks= " << totalLinks;
