...
void GraphicsWidget::drawEdge(int i, int j, float weight, bool reciprocal, bool drawArrows, QString color, bool bezier){
    ...
    EdgeWeight *edgeWeight = new  EdgeWeight (edge, 10, QString::number(weight) );
    edgeWeight-> setPos(x,y);
	edgeWeight-> setDefaultTextColor (Qt::red);
    edgeWeight-> hide();
...
/** Draws an edge from source to target Node. 
    ...
    b) when the user clicks on the AddLink button on the MW.
*/
void GraphicsWidget::drawEdge1(int i, int j, float weight, float weight1, bool reciprocal, bool drawArrows, QString color, bool bezier){
